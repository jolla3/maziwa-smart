...
// marketviewpage/pages/MainMarketView.jsx
...
import ModernHero from "./components/hero/MarketHero";
import FiltersPanel from "./components/filters/FiltersPanel";
import ModernProductCard from "./components/listings/ProductCard";
import LoadingState from "./components/states/LoadingState";
import EmptyState from "./components/states/EmptyState";

export default function MainMarketView() {
  const navigate = useNavigate();
  const { token } = useContext(AuthContext);
  ...
  useEffect(() => {
    const fetchData = async () => {
      setLoading(true);
      try {
        const listingsRes = await marketApi.fetchListings({}, token);
          ...
          <Grid container spacing={3}>
            {filteredListings.map((listing) => (
              <Grid item xs={12} sm={6} md={4} lg={3} key={listing._id}>
                <ModernProductCard listing={listing} />
              </Grid>
            ))}
          </Grid>
        )}
